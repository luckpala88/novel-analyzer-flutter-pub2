...
class AppState extends ChangeNotifier {
  ...
  void clearArcCascade({Set<String>? arcNumbers}) {
    ...
    if (!partial) {
      ...
      arcScan?.arcs.removeWhere((a) => arcNumbers.contains('${a.number}'));
  ...
  void setWritingLeanShots(bool v) {
    ...
    notifyListeners(); // v594：补notify——漏了导致精简键亮灭不刷新（点其他开关才显现）
  ...
  Future<void> extractNameMapIncrement(
    ...
    try {
      ...
      for (final line in raw.split('\n')) {
        ...
        // v540：改编新名混入左列卫兵——左名出现在起名要求里且不在生成内容中
        //=用户指定的新名被AI误当原著名（丁小丁→贺知秋事故），丢弃并记录
        if (nameReqText.contains(from) && !sourceContent.contains(from)) {
          rejected.add(from);
          continue;
        }
