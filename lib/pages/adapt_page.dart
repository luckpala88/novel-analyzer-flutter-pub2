...
class _AdaptPageState extends State<AdaptPage>
  ...
  Widget _layerTab(String label, int idx) {
    ...
    return GestureDetector(
      onTap: () => setState(() => _adaptLayer = idx),
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 5),
        decoration: BoxDecoration(
          color: selected ? const Color(0xFF92400E) : Colors.transparent,
          borderRadius: BorderRadius.circular(14),
          border: Border.all(
            color: selected ? const Color(0xFF92400E) : Colors.grey,
          ),
        ),
        child: Text(
          label,
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (scenes.isNotEmpty && _adaptLayer > 0) ...[
  ...
  Future<void> _aiSuggestSceneReq(AppState state, Arc arc, int sceneIdx) async {
    ...
    try {
      ...
      state.worldBook!.sceneRequirements[sceneReqKey] = req;
      state.saveWorldBook();
      _addLog('✓ 场景${sceneIdx + 1}改编要求已生成（${req.length}字，已填入输入框可修改）');
      if (mounted) setState(() {});
    } finally {
      if (mounted) setState(() => _isGenerating = false);
    }
