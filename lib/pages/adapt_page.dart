  ...
  Future<void> _generateBible(
    ...
    for (final arc in arcs.take(30)) {
      ...
      if (body.isEmpty) {
        ...
        for (final sc in scenes.take(5)) {
          final m = sc as Map;
          final name = (m['name'] ?? '').toString();
          final sm = (m['summary'] ?? '').toString().trim();
          if (name.isNotEmpty || sm.isNotEmpty) {
            sceneBuf.writeln('· $name：${sm.length > 120 ? sm.substring(0, 120) : sm}');
          }
        }
      ...
      materials.writeln('── 弧线${arc.number}《${item['title']}》'
          '（${item['chapter_range']}）');
      materials.writeln(body.length > 500
    ...
    if (filled == 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('弧线素材全空（无概述/场景摘要/条目），先做场景扫描')));
      return;
    }
    _addLog('📖 素材有效弧线$filled/${arcs.length}');
    final config = state.getApiConfig('wb');
    setState(() => _isGenerating = true);
    try {
