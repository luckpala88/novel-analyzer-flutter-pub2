...
class _AdaptPageState extends State<AdaptPage>
  ...
  bool _removeSceneBlock(AppState state, String arcKey, int sceneIdx) {
    ...
    entry.content = content.substring(0, thisMatch.start) + content.substring(end);
    return true;
  }
  ...
  Future<void> _generateDeclaration(AppState state, Arc arc) async {
    ...
    final arcReq =
        state.worldBook?.combinedReq(arc.number.toString()) ?? '';
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (layer == 0) ...[
                          ...
                          child: Checkbox(
                            value:
                                state.worldBook?.reqEnabled['u_$arcKey'] ??
                                    true,
                            onChanged: (v) {
                              setState(() => state.worldBook
                                  ?.reqEnabled['u_$arcKey'] = v ?? true);
                              state.saveWorldBook();
                            },
                          ),
                        ),
                        const Expanded(
                          child: Text('用户要求（勾选才发给AI）',
                    ...
                    // v589：AI优化稿框（有内容才显示）
                    if ((state.worldBook?.arcRequirementsAI[arcKey] ?? '')
                        .trim()
                        .isNotEmpty) ...[
                      const SizedBox(height: 4),
                      Row(
                        children: [
                          SizedBox(
                            ...
                            height: 22,
                            child: Checkbox(
                              value: state.worldBook
                                      ?.reqEnabled['a_$arcKey'] ??
                                  false,
                              onChanged: (v) {
                                setState(() => state.worldBook
                                    ?.reqEnabled['a_$arcKey'] = v ?? false);
                                state.saveWorldBook();
                              },
                            ),
                          ),
                          const Expanded(
                            child: Text('AI优化要求（勾选才发给AI）',
                                style: TextStyle(fontSize: 11)),
                          ),
                        ],
                      ),
                      _CollapseReqField(
                        controller: TextEditingController(
                            text: state.worldBook
                                    ?.arcRequirementsAI[arcKey] ??
                                ''),
                        labelText: 'AI优化要求（可手动修改）',
                        fontSize: 12,
                        expandLines: 5,
                        onChanged: (v) {
                      ...
                      if (layer == 0) ...[
                        ...
                        if (_declarationText(state, arc.number).isNotEmpty)
                          MiniButton(
                            ...
                            primary: false,
                            onTap: () {
                              state.worldBook?.arcDeclarations
                                  .remove(arc.number.toString());
                              state.saveWorldBook();
                              setState(() {});
  ...
  Future<void> _generateSingleSceneDecl(
      AppState state, Arc arc, int bi) async {
    final arcKey = arc.number.toString();
    final sceneReqKey = '${arcKey}_$bi';
    final entryKey = _arcEntryKey(state, arcKey);
    if (entryKey == null) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('该弧线尚无改编概述，先在"弧线改编"页生成')));
      return;
    }
  ...
  /// v589：AI优化弧线改编要求（结果写AI框+自动勾选，不动用户框）
    ...
    try {
      ...
      final result = await state.api.callApi(
        ...
        userPrompt: PromptBuilder.buildSceneReqSuggestUserPrompt(
          ...
          sceneSummary: scene.summary,
          declaration:
              state.worldBook!.sceneDeclarations[sceneReqKey] ?? '',
  ...
  Future<void> _generateForArc(
    ...
    if (layer != 'all') {
      ...
      if (layer == 'arc' && entryKey != null) {
        warn = '将清空该弧线已改编的 ${scenes.length} 个场景段与 $shotCount 个分镜，'
            '需到下层页重新生成。';
  ...
  Future<void> _generateForArcInternal(
    ...
    try {
      ...
      if (layer != 'all' && layer != 'arc') {
        ...
        _addLog('弧线${arc.number}改编概述已就绪（${layer == 'scene' ? '场景' : '分镜'}层直接使用）');
  ...
  Future<void> _generateForScene(
    ...
    try {
      ...
      // v573：场景改编声明优先（弧线声明退为兜底）
      final sceneDecl =
          state.worldBook!.sceneDeclarations['${arcKey}_$sceneIdx'] ?? declaration;
