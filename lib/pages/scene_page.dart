  ...
  void _confirmCutResume(state, int index) {
    ...
    showDialog(
      ...
      try {
        ...
        // v534：进度锚=保留末场景的endChapter——字符级续切自其end_text之后
        // （runGlobalSceneScan入口按末场景endChapter+endText定位续切位）
        state.globalSceneScannedUpTo =
            keepCount > 0 ? state.globalScenes[keepCount - 1].endChapter : 0;
        if (keepCount > 0 && !anchorOk) {
          _addLog('⚠ 保留的末场景缺end_text衔接锚点——重扫可能重复该场景章内内容，建议剪断点前移一个场景');
        }
        // 分组断点回退：已分组场景数超出保留范围则回退到剪断点
        if (state.globalGroupedUpTo > keepCount) {
          state.globalGroupedUpTo = keepCount;
          _addLog('ℹ 弧线分组断点已回退到场景$keepCount——请重新生成弧线');
        }
        state.saveGlobalScenes();
        _addLog(
          '✂ 已剪断：保留${keepCount}个场景，从第${cut.startChapter}章重扫${anchorOk ? "（衔接锚点：「${state.globalScenes[keepCount - 1].endText.length > 20 ? "${state.globalScenes[keepCount - 1].endText.substring(0, 20)}…" : state.globalScenes[keepCount - 1].endText}」）" : ""}（点批量→继续扫描场景）',
        );
      } catch (e) {
        _addLog('⛔ 剪断异常：$e');
      }
