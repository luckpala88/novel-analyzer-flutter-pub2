...
class _WritingPageState extends State<WritingPage>
  ...
  int _saveFullWriting(AppState state, WritingItem w, String oldContent) {
    ...
    if (state.writingModelNote && w.model.isNotEmpty) {
      txt = '[模型：${w.model} · 温度${w.temperature}]\n\n$txt';
    }
  ...
  Future<void> _genSingleShot(
    ...
    try {
      ...
      state.saveWritings();
      final txt = TextCleaner.stripShotHeaders(w.content);
      final path = state.storage.getWritingPath(
        w.arcKey,
        w.sceneIdx,
        w.sceneName,
  ...
  int _viewMode = 0; // 0=场景创作 1=已创作正文
                ...
                child: Row(
                  ...
                  SegmentedButton<int>(
                    ...
                    segments: const [
                      ...
                      ButtonSegment(value: 1, label: Text('已创作')),
                    ],
                    selected: {_viewMode},
  ...
  Future<void> _cleanAllWritings(AppState state) async {
    ...
    for (final k in keys) {
      ...
      final cleaned = TextCleaner.stripJsonShells(w.content);
  ...
  /// v410：弧线改编声明（开关开启才注入创作，空=未生成）
  String _arcDeclForWriting(AppState state, String arcKey) {
    if (state.worldBook?.arcDeclEnabled[arcKey] ?? true) {
      return state.worldBook?.arcDeclarations[arcKey] ?? '';
    }
    return '';
  }
  ...
  Widget _buildArcDeclSection(AppState state, String arcKey) {
    ...
    if (decl.isEmpty) return const SizedBox.shrink();
      ...
      title: Row(
        children: [
          SizedBox(
            ...
            height: 22,
            child: Checkbox(
              value: state.worldBook?.arcDeclEnabled[arcKey] ?? true,
              onChanged: (v) {
                setState(
                  () => state.worldBook?.arcDeclEnabled[arcKey] = v ?? true,
                );
                state.saveWorldBook();
              },
            ),
          ...
          Expanded(
            child: Text(
              '📄 弧线改编声明（${decl.length}字·${state.worldBook?.arcDeclEnabled[arcKey] ?? true ? "创作正文时注入" : "已停用不注入"}）',
              style: const TextStyle(
                fontSize: 11.5,
                fontWeight: FontWeight.w600,
  ...
  Widget _buildWritingCard(
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          '${w.chapterRange} · ${w.content.length}字 · v${w.version}${w.model.isNotEmpty ? ' · ${w.model}' : ''}',
          style: const TextStyle(fontSize: 11),
        ),
        trailing: Row(
          mainAxisSize: MainAxisSize.min,
  ...
  Future<String?> _generateShotByShot(
    ...
    for (var i = 0; i < shotMatches.length; i++) {
      ...
      while (true) {
        ...
        if (!result.isSuccess) {
          if (result.statusCode == 503 && shotRetry < 2) {
            shotRetry++;
            _addLog('分镜${i + 1}：503过载，${20 * shotRetry}s后重试...');
  ...
  Future<bool> _createSceneWriting(
    ...
    try {
      ...
      final userPrompt = PromptBuilder.buildWritingUserPrompt(
        ...
        freeShotMode: state.writingFreeMode,
      ...
      if (result.isSuccess) {
        ...
        // v513b：整场景原文相似度检测——场景切片对照（名称外抄袭率>10%自动改写一轮）
  ...
  List<Widget> _buildInterleavedView(
    ...
    if (jsonMode && TextCleaner.jsonFormatBad(text)) {
      text2 = TextCleaner.salvagePseudoJson(text); // 存量伪JSON剥壳+解码
    } else if (text.contains(r'\n')) {
      // 兼容模式存量字面\n（中转假流式残留）——仅解码不做其它变换
      text2 = text.replaceAll(r'\n', '\n').replaceAll(r'\"', '"');
    }
