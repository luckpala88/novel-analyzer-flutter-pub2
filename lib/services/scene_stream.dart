...
Future<void> runGlobalSceneScan({
  ...
  Future<bool> Function(String sys, String user)? previewHook,
    ...
    // v534（用户裁决：切片字符级不重不漏）：续扫统一从"最后场景end_text之后
    // 的字符位"续切——剪断后末场景收在章中间时，窗口首章喂半章；正常增量
    // 时末场景收在窗口末尾（余量为空）自然退化为下一章开头，一条规则两用
    var from = state.globalSceneScannedUpTo + 1;
    String? resumeAnchor; // 末场景end_text，仅首窗生效（后续窗口末场景必收在窗口末尾）
    if (state.globalScenes.isNotEmpty) {
      final lastSc = state.globalScenes.last;
      if (lastSc.endChapter > 0 && lastSc.endText.trim().isNotEmpty) {
        final ai = chapters.indexWhere((c) => chNumOf(chapters.indexOf(c)) == lastSc.endChapter);
        if (ai >= 0) {
          final c = chapters[ai].content;
          var p = c.indexOf(lastSc.endText);
          if (p < 0) {
            final fz = ArcText.fuzzyLocate(c, lastSc.endText);
            if (fz != null) p = c.indexOf(fz);
          }
          if (p >= 0 && p + lastSc.endText.length < c.length) {
            from = lastSc.endChapter;
            resumeAnchor = lastSc.endText;
          }
          // 余量为空（末场景收在章尾）→ 保持 from=scannedUpTo+1，即旧行为
        }
      }
    }
    log(
      from == 1
          ...
          : '继续场景扫描：从第$from章起${resumeAnchor != null ? "（字符级续切：自末场景end_text之后）" : ""}（已完成${state.globalScenes.length}场景）',
    );
    try {
      while (from <= totalChapters) {
        ...
        // 窗口文本——首章按锚点做字符级续切（v534）：定位end_text在章内的
        // 位置，从其后开始喂；定位失败降级整章+⚠（不阻断，宁重勿漏由合并层兜）
        int anchorOffset = 0;
        if (resumeAnchor != null) {
          final firstContent = chapters[fromIdx].content;
          var p = firstContent.indexOf(resumeAnchor);
          if (p < 0) {
            final fz = ArcText.fuzzyLocate(firstContent, resumeAnchor);
            if (fz != null) p = firstContent.indexOf(fz);
          }
          if (p >= 0) {
            anchorOffset = p + resumeAnchor.length;
            log('✂ 字符级续切：首章${chNumOf(fromIdx)}自锚点后${firstContent.length - anchorOffset}字开始');
          } else {
            log('⚠ 末场景end_text在${chNumOf(fromIdx)}章内定位失败——本窗口整章重喂（衔接提示继续生效，注意查重）');
          }
        }
        ...
        // v534：锚点仅首窗生效——本窗口末场景必收在窗口末尾，余量为空
        resumeAnchor = null;
        from = wTo + 1;
      }
      {
