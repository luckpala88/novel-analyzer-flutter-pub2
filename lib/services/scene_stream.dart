    ...
    try {
      ...
      if (resume && state.arcScan != null && state.arcScan!.arcs.isNotEmpty) {
        ...
        for (final a in arcs) {
          ...
          if (a.status == 'incomplete' && a.number == arcs.last.number) openArc = a;
        }
      ...
      while (cursor < gs.length) {
        ...
        final userPrompt =
            '已分组上文的未闭合弧线：$openDesc\n\n'
            '以下是场景$cursor+1-$to的序列：\n\n$sb\n\n'
            '请从未闭合弧线继续（或开始新弧线），将场景${cursor + 1}-$to归组。scene_from/scene_to用全局场景序号（1-based）。未闭合弧线延续时title保持一致（必要时加"·续"）';
        ...
        for (final aj in arcsJson) {
          ...
          final existedAnalysis = state.arcAnalyses[(isContinuation ? openArc!.number : arcNum).toString()];
