...
class TextCleaner {
  ...
  static String _fixGluedDimLines(String content) {
    ...
    for (final line in lines) {
      final t = line.trim();
      final m = dimRe.firstMatch(t);
      if (m == null) {
        out.add(line);
        continue;
      }
      final val = m.group(4) ?? '';
      final enders = RegExp(r'[。！？!?]').allMatches(val).toList();
      if (enders.length < 2) {
        out.add(line);
        continue;
      }
      // 第一个句末标点后补换行：前段=维度真值（随维度行剥离），后段=粘连正文
      final cut = enders.first.end;
      final head = t.substring(0, t.length - val.length + cut);
      final tail = val.substring(cut).trim();
      out.add(head);
  ...
  static String stripShotHeaders(String content, {bool keepModelNote = false}) {
    ...
    for (var idx = 0; idx < lines.length; idx++) {
      ...
      if (dimM != null) {
        ...
        if (!polluted && !glued.containsKey(idx)) {
          continue; // 真维度行（v262行级+v266块级双判定）
        }
        pure.add(val);
        continue;
  ...
  static String stripShotDims(String text) {
    // v612：转场手法移出精简清单——它是镜间衔接的导演指令，剥离导致割裂
    // v640：投放信息移出精简清单——它是改编后的内容指引（非易搬运行文锚）
    final re = RegExp(
      r'^[^\u4e00-\u9fa5\n]*(文笔节奏|语感锚|语感|笔墨配额|笔墨)(\s*[(（][A-Za-z /]+[)）])?(\s*/\s*[A-Za-z /]+)?\s*[：:].*$',
      multiLine: true,
    );
    var out = text.replaceAll(re, '');
    // v606：inline单行格式（分镜1｜焦点：…｜投放信息：…）——维度埋在行中间，
    // 行首正则剥不到；按全角｜切段剥掉精简4维度段（语感值内ASCII|不误切）
    final segRe = RegExp(
      r'｜\s*(文笔节奏|语感锚|语感|笔墨配额|笔墨)(\s*[(（][A-Za-z /]+[)）])?\s*[：:].*?(?=｜|$)',
    );
  ...
  static const kLabelMap = {
    ...
    '篇幅': '篇幅(Length)', '文笔节奏': '文笔节奏(Prose Style)', '语感': '语感(Voice)', '文风': '文风(Style)',
  ...
  static String stripJsonShells(String t) {
    ...
    final re = RegExp(
      ...
      // 分镜1/场景2这类键不在旧白名单里，壳连正文残留）——正文不会含JSON花括号壳，误伤概率趋零
      r'\{\s*"[^"]{1,24}"\s*:\s*"((?:[^"\\]|\\.)*)"\s*\}',
      dotAll: true,
    );
    ...
    for (final l in t.split('\n')) {
      ...
      // v644：断裂JSON裸键值行修复（Gemini 2.5实测：值含未转义引号导致
      // 整体解析失败，{"focus":"丁小丁与银衫美女…}整段原样进正文）——
      // 按行修复：维度键→中文维度行（后续stripShotHeaders剥离）；
      // 正文键(prose/content等)→值直接保留；其余键含中文→保值，纯ASCII→丢
      final kv = RegExp(r'^"([a-zA-Z_]+)"\s*[：:]\s*"?(.*)$').firstMatch(s);
      if (kv != null) {
        final key = kv.group(1)!;
        var val = (kv.group(2) ?? '').trim().replaceFirst(RegExp(r'",?\s*$'), '');
        const proseKeys = {'prose', 'content', 'body', 'text', 'output', 'result'};
        const dimKeys = {
          'focus': '焦点', 'shot_type': '镜头类型', 'pov': '视角',
          'info': '投放信息', 'intent': '作者意图', 'transition': '转场手法',
          'length': '篇幅', 'prose_style': '文笔节奏', 'voice': '语感',
          'ink': '笔墨', 'abstract': '功能抽象', 'style': '文风',
        };
        if (proseKeys.contains(key)) {
          if (val.isNotEmpty) kept.add(val);
          continue;
        }
        if (dimKeys.containsKey(key)) {
          if (val.isNotEmpty) kept.add('${dimKeys[key]}：$val');
          continue;
        }
        // 未知键：值含中文=保值（键剥掉），纯ASCII=结构残渣整行丢
        if (val.isEmpty) continue;
        if (RegExp(r'[\u4e00-\u9fa5]').hasMatch(val)) {
          kept.add(val);
        }
        continue;
      }
  ...
  /// v643：语感行专用旧阈值（≥3句末标点才判污染）
  static bool dimValuePollutedLegacy(String value) {
    final v = value.trim();
  ...
  static bool dimValuePolluted(String value) {
    ...
    if (v.isEmpty) return false;
    if (v.length > 200) return true;
    // v643：阈值3→2——DeepSeek把正文粘在维度值后（截图实证：功能抽象值
    // "建立冲突现场。人流如鲫。丁小丁蹲在…"恰2个句号漏判→正文随维度行
    // 一起被剔→69镜进补缺失）。维度行真值是短语，2个句末标点=粘连正文
    return RegExp(r'[。！？!?]').allMatches(v).length >= 2;
  }
