...
import java.io.File
import android.support.v4.media.session.MediaSessionCompat // v550: 蓝牙耳机媒体按键
...
class MainActivity : FlutterActivity() {
    ...
    private var mediaSession: MediaSessionCompat? = null // v550：TTS朗读媒体按键会话
    private var ttsMediaChannel: MethodChannel? = null
    private var audioFocusReq: android.media.AudioFocusRequest? = null // v552：音频焦点（媒体按键路由前提）
            ...
            ch.setMethodCallHandler { call, result ->
                when (call.method) {
                    "setActive" -> {
                        ...
                        setTtsMediaActive(active, playing)
                        result.success(true)
                    }
                    else -> result.notImplemented()
                }
    ...
    // v550：媒体会话管理（蓝牙耳机/线控播放暂停）
    private fun setTtsMediaActive(active: Boolean, playing: Boolean) {
        try {
            if (active) {
                if (mediaSession == null) {
                    mediaSession = MediaSessionCompat(this, "NovelAnalyzerTTS").apply {
                            ...
                            override fun onMediaButtonEvent(mediaButtonEvent: Intent): Boolean {
                                ...
                                when (ev.keyCode) {
                                    ...
                                    KeyEvent.KEYCODE_HEADSETHOOK -> {
                                        ttsMediaChannel?.invokeMethod("playPause", null)
                                        // v555：探针——让Dart弹提示确认原生收到按键
                                        ttsMediaChannel?.invokeMethod("mediaDebug", "原生收到keyCode=${ev.keyCode}")
                                        return true
                                    }
                                }
                ...
                requestTtsAudioFocus() // v552：无焦点=系统不把媒体按键路由给本APP
            } else {
                mediaSession?.isActive = false
                abandonTtsAudioFocus() // v552
            }
        } catch (e: Exception) { /* 媒体会话失败不影响朗读本身 */ }
    }

    // v552：音频焦点申请/释放——蓝牙媒体按键路由的隐含前提
    private fun requestTtsAudioFocus() {
        try {
            val am = getSystemService(Context.AUDIO_SERVICE) as android.media.AudioManager
            if (Build.VERSION.SDK_INT >= 26) {
                if (audioFocusReq == null) {
                    audioFocusReq = android.media.AudioFocusRequest.Builder(
    ...
    private fun updateTtsPlaybackState(playing: Boolean) {
        try {
            val ms = mediaSession ?: return
            val state = if (playing) PlaybackStateCompat.STATE_PLAYING else PlaybackStateCompat.STATE_PAUSED
            ms.setPlaybackState(PlaybackStateCompat.Builder()
